...
export async function selectModelBandit(taskType: TaskType, constraints?: { maxCostUsd?: number; maxLatencyMs?: number; minQuality?: number; }): Promise<RoutingResult> {
  ...
  // not enough historical data yet for this task type — fall back to
  // the Chapter 3 heuristic table until the bandit has something to learn from
  if (relevantStats.length === 0) {
    const fallback = heuristicSelectModel(taskType, constraints);
