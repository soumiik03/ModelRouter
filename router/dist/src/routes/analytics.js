...
const pool = process.env.DATABASE_URL
    ...
    : null;
function resolveWorkspaceFile(targetPath) {
...
const EXPECTED_STRATEGIES = ['always-cheap', 'always-expensive', 'heuristic-router', 'learned-bandit'];
function loadBenchmarkSummary() {
    const summaryPath = resolveWorkspaceFile('evals/results/summary.json');
    if (!summaryPath)
        throw new Error('Benchmark results are unavailable');
...
function loadRawBenchmarkRuns() {
    const rawPath = resolveWorkspaceFile('evals/results/raw-run.json');
    if (!rawPath)
        throw new Error('Benchmark run data is unavailable');
    const parsed = JSON.parse(readFileSync(rawPath, 'utf-8'));
    if (!Array.isArray(parsed))
        throw new Error('Benchmark run data is invalid');
    return parsed;
}
export default async function analyticsRoutes(app) {
    ...
    app.get('/v1/analytics', async (_req, reply) => {
        ...
        if (pool) {
            try {
                const res = await pool.query(`SELECT id, prompt, model_used as "modelUsed", cost_usd as "costUsd", 
        ...
        const effectiveLogs = dbLogs;
        const totalRequests = effectiveLogs.length;
        const failures = effectiveLogs.filter((l) => Boolean(l.error)).length;
        ...
        const totalCostUsd = effectiveLogs.reduce((acc, l) => acc + (l.costUsd || 0), 0);
        const avgLatencyMs = totalRequests > 0 ? Math.round(effectiveLogs.reduce((acc, l) => acc + (l.latencyMs || 0), 0) / totalRequests) : 0;
        // Scored logs (only non-null quality scores from live request logs)
        const scoredLogs = effectiveLogs.filter((l) => l.qualityScore != null && l.qualityScore > 0);
        const avgQualityScore = scoredLogs.length > 0
            ? Number((scoredLogs.reduce((acc, l) => acc + (l.qualityScore ?? 0), 0) / scoredLogs.length).toFixed(2))
            : null;
        const cacheHits = effectiveLogs.filter((l) => l.routingReason?.toLowerCase().includes('cache')).length;
    ...
    app.get('/v1/budgets', async (_req, reply) => {
        ...
        return reply.send({
            ...
            exceededCount: budgets.filter((b) => b.isExceeded).length,
        });
    });
}
